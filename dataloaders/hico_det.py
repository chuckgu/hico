...
class HICO(Dataset):
    def __init__(self, mode,  num_im=-1, num_val_im=5000):
        ...
        self.filenames = load_image_filenames(self.roidb_file, os.path.join(DATA_PATH, 'hico/images/'+mode) )
        ...
        self.ind_to_classes, self.ind_to_predicates = load_info(self.roidb_file)
    ...
    def __getitem__(self, index):
        ...
        # if not (gt_boxes[:, 2] >= gt_boxes[:, 0]).all():
        #     print (gt_boxes)
...
def assertion_checks(entry):
    ...
    # if not (entry['gt_boxes'][:, 2] >= entry['gt_boxes'][:, 0]).all():
    #     print(entry['gt_boxes'])
...
def load_image_filenames(image_file, image_dir):
    ...
    """
    fns = []
    for i, img in enumerate(image_file['anno']):
        # basename = '{}.jpg'.format(img['img_path'])
        basename=img['img_path']
...
def load_boxes(anno):
    ...
    for i, an in enumerate(anno['anno']):
        obj_boxes = []
        human_boxes = []
        ...
        for a in an['bbox']:
            obj_boxes.append(a['obj_box'])
            obj_cl.append(a['obj_class'])
            human_boxes.append(a['human_box'])
            human_cl.append(a['human_class'])

            hoi_cl.append(a['id'])

        obj.append(np.asarray(obj_boxes))
        human.append(np.asarray(human_boxes))
        obj_classes.append(np.asarray(obj_cl))
        human_classes.append(np.asarray(human_cl))

        hoi_classes.append(np.asarray(hoi_cl))



    return obj,obj_classes, human,human_classes,hoi_classes
...
def load_info(info):
    ...
    info['obj2id']['__background__'] = 0
    info['verb2id']['__background__'] = 0

    class_to_ind = info['obj2id']
    predicate_to_ind = info['verb2id']
...
class HICODataLoader(torch.utils.data.DataLoader):
    """
