...
def obj_vectors(names, wv_type='glove.6B', wv_dir=DATA_PATH, wv_dim=300):
    ...
    for i, token in enumerate(names):
        ...
        if wv_index is not None:
            ...
            # lw_token = sorted(token.split('_'), key=lambda x: len(x), reverse=True)
            lw_token = token.split('_')
            print(token)
            # print("{} -> {} ".format(token, lw_token))
            vectors_token=[]
            for t in lw_token:
                wv_index = wv_dict.get(t, None)
                if wv_index is not None:
                    vectors_token.append(wv_arr[wv_index])
                else:
                    print("fail on {}".format(token))

            vectors[i] = torch.stack(vectors_token).sum(0)

    return vectors

def verb_vectors(names, wv_type='glove.6B', wv_dir=DATA_PATH, wv_dim=300):
    wv_dict, wv_arr, wv_size = load_word_vectors(wv_dir, wv_type, wv_dim)
